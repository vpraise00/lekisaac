...
3. **Set up port permissions**:
   ```bash
   # Permanent (recommended, requires logout/login):
   sudo usermod -aG dialout $USER
   # Log out and back in for group change to take effect

   # Immediate (temporary, resets on reboot):
   sudo chmod 666 /dev/ttyACM0
   ```

   > **Note**: `usermod -aG dialout` is the permanent and secure approach. `chmod 666` grants device access to all users, so use it only for quick testing.
